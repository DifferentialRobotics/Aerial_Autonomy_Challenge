...
class quadrotor_dynamics: public dynamics
{
    ...
    public:
        ...
        // ~quadrotor_dynamics();external
        // quadrotor_dynamics(double mass, Matrix3d inertia) : mass_(mass), inertia_(inertia),
        //                                                     // ... 其他初始化 ...
        //                                                     external_force_(Vector3d::Zero())
        // {
        // }
        void addExternalForce(const Vector3d &force);
...
void quadrotor_dynamics::addExternalForce(const Vector3d &force)
{
    external_force_ = force;
    ROS_WARN("external force: %.2f, %.2f, %.2f",
             external_force_.x(), external_force_.y(), external_force_.z());
}
...
void quadrotor_dynamics::step_forward(double dt)
{
    ...
    Force_world = R_body2world * Force_body+ external_force_;
